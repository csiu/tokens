...
for c = 1:C
    ...
    %use Dirichlet Prior; was told features were binary
    count_x1_c = sum(X_c);
    count_c = size(X_c,1);
    p_x_c(c,:) = (count_x1_c + 1)/(count_c + 2);                 %compute P(X=1|Y=c)
    p_x_c_not(c,:) = ((count_c - count_x1_c) + 1)/(count_c + 2); %compute P(X=0|Y=c)
